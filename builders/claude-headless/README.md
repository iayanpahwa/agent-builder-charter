...
[`../../framework/`](../../framework/). This folder only adds the headless runner and the interview.

A charter lists the only things an agent may do. The runner reads it and is the only door:
model, tools, turns, timeout, network, and environment all come from the charter. **If it's not
...
- Sibling builders (`../claude-sdk/`, `../openai-agents/`, `../langchain/`) are placeholders today.
