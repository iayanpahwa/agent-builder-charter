...
Planned builder that turns a CHARTER charter into an enforced agent on LangChain / LangGraph
(deep agents). It isn't implemented yet; this folder marks the slot.

When built it will, like `../claude-headless/`:
- reuse the shared engine in `../../core/` (charter schema, `validate.py`, `eval_checks.py`),
- translate a charter into the LangChain / LangGraph config (model, tools, turn cap, MCP, system prompt, and so on),
