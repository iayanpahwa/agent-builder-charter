...
|---|---|---|
| `charter.schema.yaml` | The field-by-field spec: every charter field, its type, and an `x-enforced` tag saying how strictly it's held (R wall, CI gate, AL alarm, P present, none). This is the source of truth for what a charter contains. | no |
| `CHARTER.md` | The enforcement model in plain language: what the `x-enforced` tags mean (wall, gate, alarm, present), why a bad charter fails closed, and how the live off switch works. | no |
